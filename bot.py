...
{MAGENTA}|___/|_|_| |_| <___/ |_|  |_\_\`___'\__/|___>`___/ |_|     {MAGENTA} STAY AWARE AND KEEP LEARNING
    ...
    def main(self):
        while True:
            ...
            num_acc = len(data)
            base.log(self.line)
            base.log(f"{GREEN}Number of accounts: {WHITE}{num_acc}")

            for no, data in enumerate(data):
                base.log(self.line)
                base.log(f"{GREEN}Account number: {WHITE}{no+1}/{num_acc}")
                ...
                try:
                    ...
                    if token:
                        ...
                        base.log(f"{RED}Token not found! Please get new query id")
                except Exception as e:
                    base.log(f"{RED}Error: {WHITE}{e}")

            print()
            wait_time = 30 * 60
            base.log(f"{YELLOW}Wait for {int(wait_time/60)} minutes!")
